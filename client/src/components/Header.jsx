...
export const Header = ({ darkMode, setDarkMode }) => {
  ...
  return (
    <header className={`shadow-md transition-colors duration-300 ${darkMode ? 'bg-gray-900 text-white' : 'bg-white text-gray-900'}`}>
      ...
      {/* Menu de Editorias */}
      <nav className={`${darkMode ? 'bg-gray-800 text-gray-200 border-gray-700' : 'bg-white text-gray-800 border-gray-200'} border-b shadow-sm`}>
        <div className="container mx-auto px-4 py-2.5 flex gap-6 font-bold text-sm overflow-x-auto whitespace-nowrap">
          <Link to="/" className="hover:text-para-red transition">Início</Link>
          <span className="text-gray-400">|</span>
          <Link to="/?category=Pará" className="hover:text-para-red transition">Pará</Link>
          <span className="text-gray-400">|</span>
          <Link to="/?category=Política" className="hover:text-para-red transition">Política</Link>
          <span className="text-gray-400">|</span>
          <Link to="/?category=Economia" className="hover:text-para-red transition">Economia</Link>
          <span className="text-gray-400">|</span>
          <Link to="/?category=Esportes" className="hover:text-para-red transition">Esportes</Link>
          <span className="text-gray-400">|</span>
          <Link to="/?category=Cotidiano" className="hover:text-para-red transition">Cotidiano</Link>
          <span className="text-gray-400">|</span>
          <Link to="/?category=Cultura" className="hover:text-para-red transition">Cultura & Lazer</Link>
          <span className="text-gray-400">|</span>
          <Link to="/?category=Brasil" className="hover:text-para-red transition">Brasil</Link>
          <span className="text-gray-400">|</span>
          <Link to="/?category=Mundo" className="hover:text-para-red transition">Mundo</Link>
        </div>
      </nav>
